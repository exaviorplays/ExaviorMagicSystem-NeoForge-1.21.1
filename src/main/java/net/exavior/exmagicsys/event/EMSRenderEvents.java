...
public class EMSRenderEvents {
    ...
    public static void onRenderHand(RenderHandEvent event) {
        ...
        HumanoidArm currentHandArm;
        if (event.getHand() == InteractionHand.MAIN_HAND) {
            currentHandArm = player.getMainArm();
        } else {
            currentHandArm = player.getMainArm().getOpposite();
        }

        boolean shouldAnimate = false;
        if (spellArm == SpellArm.BOTH) {
            shouldAnimate = true;
        } else if (spellArm == SpellArm.MAIN_HAND && currentHandArm == player.getMainArm()) {
            shouldAnimate = true;
        } else if (spellArm == SpellArm.OFF_HAND && currentHandArm != player.getMainArm()) {
            shouldAnimate = true;
        }

        if (shouldAnimate) {
            SpellAnimation animation = SpellAnimations.get(animId);
            animation.applyFirstPerson(event.getPoseStack(), currentHandArm, ticksElapsed, event.getPartialTick());
        }
    }
}